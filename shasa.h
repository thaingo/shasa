...
#ifndef __shasa__shasa__
#define __shasa__shasa__
...
namespace shasa
{
    ...
    std::string to( const CONTAINER &t ) {
        ...
        for( auto it = std::begin(t); it != std::end(t); ++it ) {
            ss << shasa::to<SPEC>(*it) << SPEC::separator;
        }
    ...
    std::string to( const std::pair<K,V> &t ) {
        std::stringstream ss;
        auto key = shasa::to<SPEC>( t.first);
        auto val = shasa::to<SPEC>( t.second);
...
#   define $shasa_expand(TYPE, OPEN, CLOSE) \
...
std::stringstream ss; \
for( const auto &kv : t ) { \
auto key = shasa::to<SPEC>(kv.first); \
auto val = shasa::to<SPEC>(kv.second); \
...
return std::string() + in + ( text.size() ? ( text.back() = out, text ) : text + out ); \
    ...
    $shasa_expand( std::map, open_smap, close_smap );
    $shasa_expand( std::multimap, open_smap, close_smap );
    $shasa_expand( std::unordered_map, open_umap, close_umap );
    $shasa_expand( std::unordered_multimap, open_umap, close_umap );
#   undef $shasa_expand
    ...
    bool from( std::pair<K,V> &t, std::istream &is ) {
        ...
        if( SPEC::pair_iskv ) {
            ok = ok && shasa::from<SPEC>( t.first, is );
            ok = ok && SPEC::next( SPEC::separator_kv, is );
            ok = ok && shasa::from<SPEC>( t.second, is );
        } else {
            ok = ok && shasa::from<SPEC>( t.second, is );
            ok = ok && SPEC::next( SPEC::separator_kv, is );
            ok = ok && shasa::from<SPEC>( t.first, is );
...
do { \
if( SPEC::pair_iskv ) { \
ok = ok && shasa::from<SPEC>(val.first, is ); \
ok = ok && SPEC::next( SPEC::separator_kv, is ); \
ok = ok && shasa::from<SPEC>(val.second, is ); \
} else { \
ok = ok && shasa::from<SPEC>(val.second, is ); \
ok = ok && SPEC::next( SPEC::separator_kv, is ); \
ok = ok && shasa::from<SPEC>(val.first, is ); \
...
#   undef  $shasa_expand
    ...
    namespace {
        ...
        struct tuple {
            static std::string to( const TUPLE& t ) {
                return shasa::tuple<SPEC, TUPLE, N-1>::to(t) + SPEC::separator + shasa::to<SPEC>( std::get<N-1>(t) );
            }
            static bool from( TUPLE& t, std::istream &is ) {
                bool ok = true;
                ok = ok && shasa::tuple<SPEC, TUPLE, N-1>::from( t, is );
                ok = ok && SPEC::next( SPEC::separator, is );
                ok = ok && shasa::from<SPEC>( std::get<N-1>(t), is );
                return ok;
            }
        };
        ...
        struct tuple<SPEC, TUPLE, 1> {
            static std::string to( const TUPLE& t ) {
                return shasa::to<SPEC>( std::get<0>(t) );
            }
            static bool from( TUPLE& t, std::istream &is ) {
                return shasa::from<SPEC>( std::get<0>(t), is );
            }
        };
    }
    ...
    std::string to( const std::tuple<Args...>& t ) {
        std::stringstream ss;
        ss << SPEC::open_seq;
        ss << shasa::tuple<SPEC, decltype(t), sizeof...(Args)>::to(t);
        ss << SPEC::close_seq;
        return ss.str();
    }
    template<class SPEC, class... Args>
    bool from( std::tuple<Args...> &t, std::istream &is ) {
        ...
        if(  ok && SPEC::next( SPEC::close_seq, is ) ) return ok;
        ok = ok && SPEC::prev( SPEC::close_seq, is );
        ok = ok && shasa::tuple<SPEC, decltype(t), sizeof...(Args)>::from(t, is );
        ok = ok && SPEC::next( SPEC::close_seq, is );
        return ok;
    }
...
#define SHASA_DEFINE( OBJECT, PARGS ) \
namespace shasa { \
template<class SPEC> \
std::string to( const OBJECT ) { \
auto _tpl = std::make_tuple PARGS; \
return shasa::to<SPEC>( _tpl ); \
} \
template<class SPEC> \
bool from( OBJECT, std::istream &is ) { \
auto _tpl = std::make_tuple PARGS; \
return shasa::from<SPEC>( _tpl, is ) ? (std::tie PARGS = _tpl, true) : false;  \
} \
}
...
namespace shasa {
    ...
    template <typename T>
    std::string to_shasa( const T &t ) {
        return to<specs<SHASA>>( t );
    }
    ...
    template <typename T, typename ISTREAM >
    bool from_shasa( T &t, ISTREAM &is ) {
        return from<specs<SHASA>>( t, is );
    }
}
...
namespace std {
    using shasa::to_json;
    using shasa::from_json;
}
#endif

#endif /* defined(__shasa__shasa__) */
